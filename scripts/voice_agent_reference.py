...
class ToolProcessor:
    ...
    async def _run_tool(self, tool_name, tool_content):
        ...
        if tool == "getdateandtimetool":
            ...
            debug_print("TrackOrderTool starting operation that will take time...")
            ...
            seed = (
                int(hashlib.md5(order_id.encode(), usedforsecurity=False).hexdigest(), 16) % 10000
            )
            random.seed(seed)
            ...
            if status == "Delivered":
                ...
                estimated_delivery = (today + datetime.timedelta(days=delivery_days)).strftime(
                    "%Y-%m-%d"
                )
            ...
            debug_print("TrackOrderTool completed successfully")
...
class BedrockStreamManager:
        ...
        get_order_tracking_schema = json.dumps(
            {
                "type": "object",
                "properties": {
                    "orderId": {"type": "string", "description": "The order number or ID to track"},
    ...
    async def initialize_stream(self):
        ...
        try:
            ...
            self.is_active = False
            print(f"Failed to initialize stream: {e!s}")
            raise
    ...
    async def send_raw_event(self, event_json):
        ...
        try:
            ...
            debug_print(f"Error sending event: {e!s}")
            if DEBUG:
                import traceback
                        ...
                        try:
                            ...
                            if "event" in json_data:
                                if "completionStart" in json_data["event"]:
                                    ...
                                    and json_data["event"].get("contentEnd", {}).get("type")
                                    == "TOOL"
                                ):
    ...
    def _handle_tool_task_completion(self, task, content_name):
        ...
        if task.done() and not task.cancelled():
            ...
            if exception:
                debug_print(f"Tool task failed: {exception!s}")
    ...
    async def _execute_tool_and_send_result(
        ...
        try:
            ...
            debug_print(f"Error executing tool {tool_name}: {e!s}")
            ...
            try:
                ...
                debug_print(f"Failed to send error response: {send_error!s}")
...
class AudioStreamer:
    ...
    def input_callback(self, in_data, frame_count, time_info, status):
        ...
        if self.is_streaming and in_data:
            ...
            asyncio.run_coroutine_threadsafe(self.process_input_audio(in_data), self.loop)
    ...
    async def play_output_audio(self):
        ...
        while self.is_streaming:
            try:
                ...
                if self.is_streaming:
                    print(f"Error playing output audio: {e!s}")
                    import traceback
